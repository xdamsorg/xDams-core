...
function lastIndexOf(str, srch, startIdx){
	...
	for(idx = startIndex; idx >= 0; idx--){
		if(idx + srch.length <= str.length){
			if(str.substr(idx, srch.length) === srch) break;
		}
	}
...
$.fn.autocomplete = function(options){ return this.each(function(){
  ...
  var zIndex = 99999;//me.css('z-index');/*diego*/
	
  function getStartEndInputPosition(val){
  	  var separators = options.multi_separators;
      var pos = getCaretPosition(me_this);
      
      var start = 0;
      for(var i = 0; i < separators.length; i++){
      	var idx = lastIndexOf(val, separators[i], pos.start);
      	if(idx >= 0 && (pos.start - idx) < (pos.start - start)){
      		start = idx + separators[i].length;
      	}
      }
      
      var end = val.length;
      for(var i = 0; i < separators.length; i++){
      	var idx = val.indexOf(separators[i], pos.start);
      	if(idx >= 0 && (idx - pos.start) < (end - pos.start)){
      		end = idx;
      	}
      }
  ...
  function getUserInput(){
    ...
    if(options.multi){
      ...
      var startEnd = getStartEndInputPosition(val);
  ...
  function setSuggestion(val){
    ...
    if(options.multi){
      ...
      */
      ...
      var orig = me.val();
      var startEnd = getStartEndInputPosition(orig);
	  var start = startEnd[0];
	  var end = startEnd[1];
