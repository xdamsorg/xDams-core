...
<form name="inc_theForm" class="srcForm" action="${contextPath}/search/<%=workFlowBean.getAlias() %>/title-multiarchive.html" onsubmit="document.inc_theForm['<%=ricercaLibera%>'].value=='<spring:message code="ricerca_libera" text="ricerca libera"/>'?document.inc_theForm['<%=ricercaLibera%>'].value='':void(0)" method="post"><%
String incRicQrID = "";
...
<input type="submit" class="cerca_b" title="&gt;" value="&gt;" /></form>
